...
export interface ServiceEmail {
  id: string;
  service_type: 'manager' | 'maintenance' | 'visitors' | 'rooms' | 'rooms_special' | 'rooms_tic' | 'parking' | 'transport';
  email: string;
  created_at?: string;
}
...
export const settingsService = {
  ...
  async syncLocalEmails(): Promise<ServiceEmail[]> {
    ...
    try {
      ...
      console.warn('Error al sincronizar correos locales con el servidor:', e);
    }
    return [];
  },
